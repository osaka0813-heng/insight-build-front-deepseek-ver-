...
function scopeOf(raw: LocalizedInsightEdition): InsightScope {
  return raw.scope ?? 'global';
}

function insightsForScope(scope: InsightScope): LocalizedInsightEdition[] {
  return insights().filter((item) => scopeOf(item) === scope);
}

export const insightRepository = {
  getCurrent(language: LanguageCode, scope: InsightScope = 'global'): InsightEdition {
    const current = [...insightsForScope(scope)]
    ...
    if (!current) throw new Error(`Current Insight is missing for scope: ${scope}.`);
    return resolve(current, language);
  },
  getCurrentForScope(language: LanguageCode, scope: InsightScope): InsightEdition | undefined {
    const current = [...insightsForScope(scope)]
      .filter(isRenderableInsight)
      .sort(
        (a, b) =>
    ...
    return current ? resolve(current, language) : undefined;
  },
  hasScope(scope: InsightScope): boolean {
    return insightsForScope(scope).some(isRenderableInsight);
  },
